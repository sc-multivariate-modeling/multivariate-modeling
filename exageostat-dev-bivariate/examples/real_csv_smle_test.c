...
int main(int argc, char **argv) {
	...
	if(strcmp (data.computation, "exact") == 0)
		MORSE_smle_Call(&data, ncores, gpus,
				dts, p_grid, q_grid, 
				N,  nZobs, nZmiss);
	//else if (strcmp (data.computation, "diag_approx") == 0)
	//MORSE_smle_diag_Call(&data, ncores, gpus, dts, p_grid, q_grid, N,  nZobs, nZmiss);		
	//#if defined(EXAGEOSTAT_USE_HICMA)
	//else if (strcmp (data.computation, "lr_approx") == 0)
	//{
	//HICMA_smle_Call(&data, ncores, gpus, lts, p_grid, q_grid, N,  nZobs, nZmiss);
	//if (test == 1)
	...
	if(strcmp (data.computation, "exact") == 0 || strcmp (data.computation, "diag_approx") == 0)
		MORSE_MLE_szcpy(&data, streamdatas);
#if defined(EXAGEOSTAT_USE_HICMA)
	else if (strcmp (data.computation, "lr_approx") == 0)	
		HICMA_MLE_zcpy(&data, streamdatas);				
#endif
	if(log == 1 && test == 1)
		init_log(&data);                 
	...
	if(nZmiss != 0){
		...
		for (j = 0; j < 1; j++)
		{
			...
			if (strcmp (data.computation, "exact") == 0)
				prediction_error = MORSE_smle_Predict_Tile(&data, starting_theta, nZmiss,
						nZobs, Zobs, Zactual,
						Zmiss, N);
			else if (strcmp (data.computation, "diag_approx") == 0)
				prediction_error = MORSE_dmle_diag_Predict_Tile(&data, starting_theta, nZmiss,
						nZobs, Zobs, Zactual, 
						Zmiss, N);
#if defined(EXAGEOSTAT_USE_HICMA)
			else if (strcmp (data.computation, "lr_approx") == 0)
				prediction_error = HICMA_dmle_Predict_Tile(&data, starting_theta, nZmiss, 
						nZobs, Zobs, Zactual, 
						Zmiss, N, lts);
#endif
	...
	print_result(&data, starting_theta, N, 
			zvecs, ncores, dts,
			test, arguments.ikernel, data.computation, 
			p_grid, q_grid, data.final_loglik,
			prediction_error);
