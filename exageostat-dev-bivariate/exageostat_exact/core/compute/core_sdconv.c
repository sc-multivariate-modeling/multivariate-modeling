...
/***************************************************************************//**
 *
 *  core_dsconv - Generate covariance matrix A in dense format between two sets of locations (l1, l2) (Matern Kernel).
 ...
 *
 ******************************************************************************/
void core_sdconv (float *A, double *B, int m, int n) {

	int i, j;
	char buf[42];
	for (j = 0; j < n; j++) {
		for (i = 0; i < m; i++) {
			sprintf(buf, "%.8g", A[i + j * m]); // round to 7 decimal digits
			B[i + j * m] =  atof(buf) ;
