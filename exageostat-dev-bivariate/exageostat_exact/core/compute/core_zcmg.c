 ...
 *
 ******************************************************************************/
void core_scmg (float *A, int m, int n, int m0, int n0, location  *l1, location *l2, double *localtheta, int distance_metric) {
...
void core_sdcmg (double *A, int m, int n, int m0, int n0, location  *l1, location *l2, double *localtheta, int distance_metric) {
...
void core_dcmg_bivariate_flexible (double *A, int m, int n, int m0, int n0, location  *l1, location *l2, double *localtheta, int distance_metric) {
	...
	for (i = 0; i < m; i+=2) {
		...
		y0 = l1->y[i0];
		for (j = 0; j < n; j+=2) {
			expr1 = calculateDistance(x0, y0, l2->x[j0], l2->y[j0], distance_metric)/scale1;
			expr2 = calculateDistance(x0, y0, l2->x[j0], l2->y[j0], distance_metric)/scale2;
			expr12 = calculateDistance(x0, y0, l2->x[j0], l2->y[j0], distance_metric)/scale12;
			...
			if(expr1 == 0){
				...
				A[i + j * m] = con1 * pow(expr1, nu1)*gsl_sf_bessel_Knu(nu1, expr1); 
				A[(i + 1) + j * m] = A[i + (j + 1) * m] = con12 * pow(expr12, nu12) * gsl_sf_bessel_Knu(nu12, expr12);  
				A[(i + 1) + (j + 1) * m] = con2 * pow(expr2, nu2)*gsl_sf_bessel_Knu(nu2, expr2);
			}
			j0++;
		}
		i0++;
 ...
 *
 ******************************************************************************/
void core_dcmg_bivariate_parsimonious (double *A, int m, int n, int m0, int n0, location  *l1, location *l2, double *localtheta, int distance_metric) {
...
//	double localtheta[0] = localtheta[0], localtheta[1] = localtheta[1];
	...
	for (i = 0; i < m-1; i+=2) {
		...
		y0 = l1->y[i0];
		for (j = 0; j < n-1; j+=2) {
			expr = calculateDistance(x0, y0, l2->x[j0], l2->y[j0], distance_metric)/localtheta[2];

			if(expr == 0){
				A[i + j * m] = localtheta[0] ;
				A[(i + 1) + j * m] = A[i + (j + 1) * m] = rho * sqrt(localtheta[0] * localtheta[1]) ;
				A[(i + 1) + (j + 1) * m] = localtheta[1] ;
			}else{
				A[i + j * m] = con1 * pow(expr, localtheta[3])*gsl_sf_bessel_Knu(localtheta[3], expr); 
				A[(i + 1) + j * m] = A[i + (j + 1) * m] = con12 * pow(expr, nu12) * gsl_sf_bessel_Knu(nu12, expr);  
				A[(i + 1) + (j + 1) * m] = con2 * pow(expr, localtheta[4])*gsl_sf_bessel_Knu(localtheta[4], expr);
			}
			// printf ("===%d, %d, %d, %d (%f)\n" , i, j, i0, j0, expr);
			j0++;
		}
...
void core_dcmg_deformation (double *A, int m, int n, int m0, int n0, location  *l1, location *l2, double *localtheta, int distance_metric) {
