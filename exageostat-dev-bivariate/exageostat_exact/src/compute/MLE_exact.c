...
double MORSE_dmle_Tile(unsigned n, const double * theta, double * grad, void * MORSE_data) {
	...
	else
	{
		...
		if(strcmp(data->kernel_fun, "bivariate_matern_parsimonious2")   == 0 || strcmp(data->kernel_fun, "bivariate_matern_parsimonious2_profile")   == 0)
		{
			...
			MORSE_MLE_dcmg_Tile_Async(MorseUpperLower, MORSE_descsubC11, &data->l1, &data->l1, &data->lm, univariate_theta, data->dm, "univariate_matern_stationary",  msequence, &mrequest[0]);
			...
			univariate2_theta[2]=nu12; 
			MORSE_MLE_dcmg_Tile_Async(MorseUpperLower, MORSE_descsubC12, &data->l1, &data->l1, &data->lm, univariate2_theta, data->dm, "univariate_matern_stationary",  msequence, &mrequest[0]);
	...
	VERBOSE(" Done.\n");
			...
			MORSE_MLE_dcmg_Tile_Async(MorseUpperLower, MORSE_descsubC22, &data->l1, &data->l1, &data->lm, univariate3_theta, data->dm, "univariate_matern_stationary",  msequence, &mrequest[0]);
			//MORSE_Sequence_Wait(msequence);
		}
		else   
...
//			checkpointing(data->checkpoint_file, data->iter_count, theta, loglik, num_params);
	...
	//	fprintf(stderr,"\n------logdet: %.17g ", logdet);
		...
		{	
			...
			if(data->log == 1)
				fprintf(data->pFileLog,"%2.6f, %2.6f,", data->variance1, data->variance2);
			i = 2;
		}
		else
			i=0;
		for(;i<num_params; i++)
		{
			...
			if(data->log == 1)
				fprintf(data->pFileLog,"%2.6f, ", theta[i]);
		}

		printf(")----> LogLi: %.17g\n", loglik);
		if(data->log == 1)
			fprintf(data->pFileLog, ")----> LogLi: %.17g\n", loglik);
	 ...
	 * */
{
	...
	free(truthalpha);
        ...
        {
#endif
                pFile = fopen("mloe-mmom.txt","a");

                if(pFile == NULL) {
                        printf("Cannot access the results path(3)\n");
                        exit(0);
                }
                fprintf(pFile, "%f ", data->mloe);
                fprintf(pFile, "%f ", data->mmom);
                fprintf(pFile, "%s \n", name);        
        fclose(pFile);
#if defined(CHAMELEON_USE_MPI)
        }
